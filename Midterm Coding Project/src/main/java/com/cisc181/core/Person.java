...
public abstract class Person implements java.io.Serializable {
	...
	public void checkPhone(Person p) throws PersonException {
		String regex = "^\\(?([0-9]{3})\\)?[-.\\s]?([0-9]{3})[-.\\s]?([0-9]{4})$";
		Pattern pattern = Pattern.compile(regex);
		Matcher matcher = pattern.matcher(this.phone_number);
		if (matcher.matches() == false) {
			System.out.println("The phone number you entered is of illegal format.");
	...
	public void checkAge(Person p) throws PersonException {
		Calendar today = Calendar.getInstance();
		Calendar birthDate = Calendar.getInstance();
		birthDate.setTime(this.DOB);

		if (today.get(Calendar.YEAR) - birthDate.get(Calendar.YEAR) >= 100) {
			System.out.println("You can't have an age over 100 years old.");
			throw new PersonException();
